...
function removeUser($input = array()) {
	...
	$logger = new Logger("/usr/local/nimbusec/nimbusec/hooks/logs", "remove.log", true);
	$data = $input ['data'];
	...
	try {
		...
		$hasNimbusec = array_key_exists ( 'nimbusec_bundles', $pkgRes['data']['pkg'] );
		$logger->debug("User has nimbusec [has_nimbusec] '{$hasNimbusec}'");
