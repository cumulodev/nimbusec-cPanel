...
if [ ! -d /usr/local/cpanel/whostmgr/docroot/cgi/addons ]; then
	mkdir -p /usr/local/cpanel/whostmgr/docroot/cgi/addons
fi
if [ ! -d /usr/local/cpanel/whostmgr/docroot/cgi/addons/nimbusec ]; then
	mkdir -p /usr/local/cpanel/whostmgr/docroot/cgi/addons/nimbusec
fi
if [ ! -d /usr/local/nimbusec ]; then
...
if [ `ls -l /usr/local/nimbusec/lib/*.php | wc -l` -eq 8 ]; then
...
if [ -f /usr/local/nimbusec/nimbusec/install.php ]; then
...
if [ -x /usr/local/cpanel/bin/register_appconfig ]; then
	...
	if [ \( -f /usr/local/cpanel/whostmgr/docroot/cgi/addons/nimbusec/nimbusec.cgi \) -a \( -f /usr/local/cpanel/whostmgr/docroot/cgi/addons/nimbusec/nimbusec.php \) ]; then
		...
		rm -f /usr/local/cpanel/whostmgr/docroot/cgi/nimbusec.php >>$LOGFILE 2>&1
	    	...
	    	if [ `/usr/local/cpanel/bin/register_appconfig conf/nimbusec.conf | tee -a $LOGFILE  | wc -l` -eq 1 ]; then
	...
	if [ \( -f /usr/local/cpanel/whostmgr/docroot/cgi/addon_nimbusec.cgi \) -a \( -f /usr/local/cpanel/whostmgr/docroot/cgi/nimbusec.php \) -a \( -f /var/cpanel/apps/nimbusec.conf \) ]; then
		echo "INFO: Backend files installed" | tee -a $LOGFILE
	else
		echo "ERROR: Copying backend files failed! The installation will be aborted." | tee -a $LOGFILE
		exit 1
...
if [ ! -f /usr/local/cpanel/whostmgr/docroot/addon_plugins/nimbusec-icon.png ]; then
