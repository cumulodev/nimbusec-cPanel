...
echo "INFO: Removing cPanel conf files for x3 at /usr/local/cpanel/bin/nimbusec" | tee -a $LOGFILE
...
if [ -x /usr/local/cpanel/bin/unregister_appconfig ]; then
		if [ `/usr/local/cpanel/bin/unregister_appconfig conf/nimbusec.conf | tee -a $LOGFILE  | wc -l` -eq 1 ]; then
		...
		if [ -f /var/cpanel/apps/nimbusec.conf ]; then
			rm -f /var/cpanel/apps/nimbusec.conf >> $LOGFILE 2>&1
		fi
else 
	if [ \( ! -f /usr/local/cpanel/whostmgr/docroot/cgi/addon_nimbusec.cgi \) -a \( ! -f /usr/local/cpanel/whostmgr/docroot/cgi/nimbusec.php \) ]; then
		if [ -f /var/cpanel/apps/nimbusec.conf ]; then
			rm -f /var/cpanel/apps/nimbusec.conf >> $LOGFILE 2>&1
		fi
